...
extern uint16_t led_patterns[LED_PORTS][LED_NUM][LED_CHANNELS];
...
int cmd_parser(uint8_t *buf){
    ...
    int j = 0;
    int k = 0;
    uint32_t tmp_br_level = 0x40;
    printf("%s\n", __func__);

    if(strstr(buf, "set_br")){
    	sscanf(buf, "%s %d", cmd, &tmp_br_level);
#if 0
        tmp_br_level = tmp_br_level*4;
        if(tmp_br_level > 65535){
            br_level = 65535;
        }else{
            br_level = tmp_br_level & 0xffff;
        }
	    printf("br_level= 0%x\n", br_level);
#else
